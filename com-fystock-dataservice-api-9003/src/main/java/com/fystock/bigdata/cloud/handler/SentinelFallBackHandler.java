...
public class SentinelFallBackHandler {
     ...
     *
     * @param tradeDate 交易日参数
     * @param t         抛出的异常
     */
    public static CommonResult<List<RealTimeMarketCalculation>> getMarketDataByTradeDateFallbackHandler(String tradeDate, Throwable t) {
        if (t instanceof RuntimeException) {
            log.error("-------->查询交易日为：" + tradeDate + "行情数据异常回调处理！RuntimeException异常信息：" + t.getMessage());
        }
        return CommonResult.failed("-------->查询交易日为：" + tradeDate + "行情数据异常回调处理！RuntimeException异常信息" + t.getMessage());
    }
}
